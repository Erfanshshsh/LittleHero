...
public static class GameProgressManager
{
    ...
    {
        PlayerPrefs.SetString($"StartedZone_{difficulty}", location.ToString());
        PlayerPrefs.Save();
    }

    public static void DeleteZoneStarted(Common.Difficulty difficulty)
    {
        PlayerPrefs.DeleteKey($"StartedZone_{difficulty}");
        PlayerPrefs.Save();
    }
