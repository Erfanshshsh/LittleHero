...
public class MenuListener : MonoBehaviour {
	...
	private void Awake()
	{
		ImagePath = Application.persistentDataPath + "/EX";  //本地图片包文件夹
	}
	...
	void Start() {
		if (Application.loadedLevel == Utils.MainInterface) {					//如果当前场景为主界面场景
			...
			if(Utils.ExperimentalType == 1) {
				transform.Find ("EXInterduction").gameObject.SetActive (false);
			...
			if(introductionIndex != 0) {
				IntroductionUI.transform.Find ("Previous").gameObject.SetActive (true);
			}
	...
	public void MusicSwitch() {
		...
		if (Utils.isMusic) {
			AudioPanel.transform.Find ("MusicSwitch").GetComponent<Image> ().sprite = sprites [0];
		} else {
			AudioPanel.transform.Find ("MusicSwitch").GetComponent<Image> ().sprite = sprites [1];
		}
	}
	...
	public void AudioSwitch() {
		...
		if (Utils.isAudio) {
			AudioPanel.transform.Find ("AudioSwitch").GetComponent<Image> ().sprite = sprites [2];
		} else {
			AudioPanel.transform.Find ("AudioSwitch").GetComponent<Image> ().sprite = sprites [3];
		}
	}
	...
	public void updateButton() {
		if(Utils.localEXList.Count > Utils.BuiltInCounts) {
			...
			for(int i = 6; i < Utils.localEXList.Count; i++) {
				...
				temp.transform.SetParent(transform.Find("PipeClamp"));						//设置父物体
				temp.transform.name = ((string[])Utils.localEXList[i])[0];						//设置名称
				temp.transform.localScale = new Vector3(1, 1, 1);								//设置缩放比
				temp.transform.Find("Text").transform.GetComponent<Text>().text 
				...
				for (int j = 0; j < rgbList.Count; j++) {
					if (temp.transform.name.Equals (((string[])rgbList [j]) [0])) {
						...
						temp.transform.Find("Text").transform.GetComponent<Text>().color = 
							new Color(r, g, b, 1.0f);													//更改字体颜色
					}
				}
				
				temp.transform.Find("Image").transform.GetComponent<Image>().sprite = 
	...
	public void suitLayout() {
		count = transform.Find("PipeClamp").childCount;									//获取总共实验按钮个数
		...
		for(int i = 0; i < count; i++) {
			buttons [i] = transform.Find("PipeClamp").GetChild (i).gameObject;
		}
		switch(count) {
		case 7:
